...
function upload_ezpac_mongo_singleblock(eeg_name, orig_fname,patient_id,user_file_block,ref)
...
if ref == 0
        ref=0;
        fname=[eeg_name '_mp_1.mat'];
        mongo_upload_hfos(conn,fname,orig_fname,patient_id,ref,user_file_block)
        mongo_upload_electrodes(conn,fname,orig_fname,patient_id,user_file_block)
end;

if ref == 1
        ref=1;
        fname=[eeg_name '_bp_1.mat'];
        mongo_upload_hfos(conn,fname,orig_fname,patient_id,ref,user_file_block)
        mongo_upload_electrodes(conn,fname,orig_fname,patient_id,user_file_block)
end;
